...
def run_tool(countries, field_name, pvo_path, npp_path, km2_MW, density, proportion, increment,
             output_rasters, output_path, targets, verbose=False):
    ...
    try:
        ...
        for prop in arange(proportion, 1.00001, proportion) if increment else [proportion]:
            try:
                ...
                for _, row in targets.iterrows():
                    try:
                        ...
                        # get target for this country (and only continue if valid)
                        ...
                        if target > pvo_total:
                            ...
                            exceeded_countries.append({'ISO_3':iso3, 'increment':increment})
            ...
            finally:    # this means we don't lose the data if an error does appear
                arcpy.AddMessage(f"\n {output_csv_data} \n")
...
if __name__ == "__main__":
    ...
    output_path = arcpy.GetParameterAsText(9)
    target_file = arcpy.GetParameterAsText(10)
    ...
    run_tool(countries_shp, field_name, pvo_raster, npp_raster, km2_MW, density, proportion, 
             increment, output_rasters, output_path, targets, verbose=False)